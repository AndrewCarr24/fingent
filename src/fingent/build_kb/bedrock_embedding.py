...
class BedrockTitanEmbedding(Embedding):
    """Amazon Bedrock Titan v2 via boto3.

    Single-call per text (Titan's runtime API doesn't batch), so embedding
    throughput is bounded by Bedrock's request rate, not ours.
    """
